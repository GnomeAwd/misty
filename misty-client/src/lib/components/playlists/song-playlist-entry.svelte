...
<div class="hover:bg-card group flex items-center justify-between gap-4 rounded-lg px-2 py-1 w-full">
	<div class="flex items-center justify-start gap-4">
		<div class="flex h-12 w-12 items-center justify-center">
            <img src={albumArtUrl || '/default-album-art.png'} alt="Album Art" class="h-10 w-10 rounded-md object-cover"/>
		</div>
        <div class="flex flex-col gap-2 items-start justify-center">
            <span class="text-sm font-semibold">{song.title || 'Unknown Song'}</span>
            <span class="text-xs text-muted-foreground">{artistName || 'Unknown Artist'}</span>
        </div>
	</div>
